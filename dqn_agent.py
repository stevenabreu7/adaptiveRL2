...
def run_experiment(cfg: ExpCfg, train_dir: str, seed=0):
    ...
    checkpoint_manager = orbax.checkpoint.CheckpointManager(
        train_dir, checkpointer, options
    )
    ...
    epsilon_by_frame = optax.polynomial_schedule(
        *cfg.eps_schedule, transition_steps=cfg.n_episodes
    )
    trainstate = train_state.TrainState.create(
        apply_fn=network.apply,
        params=network.init(key, jnp.expand_dims(state, axis=0)),
        tx=optimizer,
    )
    ...
    for episode in tqdm(range(cfg.n_episodes)):
        ...
        for t in range(cfg.n_steps):
            ...
            if len(replay_buffer) > cfg.batch_size:
                ...
                trainstate, loss = train_step(
                    trainstate, target_params, batch, cfg.gamma
                )
        ...
        checkpoint_manager.save(
            episode + 1, chkpt, save_kwargs={"save_args": save_args}
        )

    return (
        trainstate.params,
        jnp.array(reward_per_episode),
        jnp.array(mean_loss_per_episode),
    )
...
def eval_agent(cfg: ExpCfg, params, epsilon=0.05, seed=0):
    ...
    netstate = train_state.TrainState.create(
        apply_fn=network.apply, params=params, tx=optax.adam(0.0)
    )
...
class DQNAgent:
    def __init__(self, cfg: ExpCfg, seed=0) -> None:
        ...
        # training state
        self.eps_fn = optax.polynomial_schedule(
            *cfg.eps_schedule, transition_steps=cfg.n_episodes
        )
        ...
        for episode in tqdm(range(self.cfg.n_episodes)):
            ...
            for t in range(self.cfg.n_steps):
                ...
                if len(self.replay_buffer) > self.cfg.batch_size:
                    ...
                    self.trainstate, loss = train_step(
                        self.trainstate, self.target_params, batch, self.cfg.gamma
                    )
