...
class BackpropamineRNN(nn.Module):
    ...
    def __init__(self, isize, hsize, osize, freeze_plasticity=False):
        ...
        self.h2o = torch.nn.Linear(hsize, osize)    # From recurrent to outputs (e.g. action probabilities in RL)
    ...
    def forward(self, inputs, hidden_hebbian):
        ...
        h_next = F.tanh(
            self.i2h(inputs) + h_cur.unsqueeze(1).bmm(self.w + torch.mul(self.alpha, hebb)).squeeze(1)
        )  # Update the h-state
        ...
        deltahebb = torch.bmm(h_cur.unsqueeze(2), h_next.unsqueeze(1))  # Batched outer product of previous hidden state with new hidden state
        ...
        self.clipval = 2.0
        hebb_next = torch.clamp(hebb + myeta * deltahebb, min=-self.clipval, max=self.clipval)
