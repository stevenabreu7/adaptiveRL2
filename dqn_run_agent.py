...
trial_name = "trial-{}-{}e-{}s-{}lr-{}g-{}fr-{}bs-{}rs".format(
    "x".join(map(str, cfg.architecture)),
    cfg.n_episodes,
    cfg.n_steps,
    cfg.learning_rate,
    cfg.gamma,
...
random.seed(seed)
print(
    "Trial #{} with {} episodes (max {} steps)".format(
        seed, cfg.n_episodes, cfg.n_steps
    )
)
### agent = DQNAgent(cfg, seed=seed)

# returns reward and mean loss per episode
params, reward, loss = run_experiment(
    cfg, train_dir=f"dqn_results/{trial_name}", seed=seed
)
